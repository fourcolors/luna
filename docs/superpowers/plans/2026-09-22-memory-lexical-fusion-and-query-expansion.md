...
Per-call judge time on the laptop (indicative only; median / p95, depth 40): `jev` 0.21 / 0.33 s, `ce` 2.2 / 2.8 s, `haiku` 2.9 / 3.6 s.
A per-turn rerank gets at most 1.5 s (`resolveRecallRerankTimeoutMs`, less when retrieval has used part of the 2.5 s recall window); calls over 1.5 s at depth 40: `jev` 0 of 200, `ce` 195, `haiku` 200 (at depth 20: 1, 12, 191).
...
Configured, Jev reranks both `memory_search` and per-turn recall (flag `0` turns a lane off), judges the top 40, pins `jev-1.13.0`, and sends the held-out request (one builder in `@luna/core`, identical to the benchmarked request except the model id: the held-out run asked for `jev-latest`, and `jev-1.13.0` served every one of its calls).
...
Gating the tuning sets offline (Jev at depth 40; scores in `packages/memory/bench/results/2026-09-23-search-tuning/jev-threshold-scores.json`, tables from `jev-threshold.py` there).
The LongMemEval scores come from a rerun of the locked Jev setup on q1-60, which put 85 evidence turns in the top 5 against 86 in `longmemeval-tuning-judges-jev-v2-wording.json` (one question, `561fabcd`, differs; not isolated whether Jev or near-tied retrieval moved):
...
The cross-encoder's 40 would have thrown away 13 of those 85 evidence turns.
Threshold 5 loses nothing on either tuning set and cuts memory-suite's negative queries that still inject a memory from 40 of 40 to 22; it needs a held-out check before it becomes the default (`LUNA_RERANK_THRESHOLD=5` applies it now).
