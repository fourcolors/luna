> **Status: shipped** — historical design record, brought onto master for the design trail; not current truth. The #1a readiness gate + boot-branch landed on master, which is the source of truth for behavior.
