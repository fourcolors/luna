...
6. **Survey surface scope = TUI-first (D-decision-1).** D3 builds the TUI surface only; web/Tauri follow once the verdict→backend contract is proven live. This is the smallest boot-risk surface and matches the operator's prior choice (survey UI = TUI-first).
