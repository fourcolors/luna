> **Status: shipped** — historical design record, brought onto master for the design trail; not current truth. The setup-mode onboarding (readiness gate #1a + login terminal #1b) landed on master, which is the source of truth for behavior.
