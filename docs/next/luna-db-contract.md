...
1. **Boot against a copy.** Copy `luna.db`, `memory.db` (with its `.hnsw.bin` sidecar if present), and every active workspace's `workspace.db` to a scratch location.
   Boot the new daemon against the copies only.
   If the new daemon's migration chain does anything other than a clean no-op `applyMigration` skip against a live-shaped copy, the gate fails before step 2 even runs.
2. **Replay the retrieval bench.** Run `packages/memory`'s `bun run bench:memory` (the retrieval bench harness referenced by `NEXT.md`'s Decision 5) against the copied `memory.db`.
   A cutover that silently reinterprets `embedding_format`, drops the `enrichment` FTS column, or otherwise changes ranking behavior must show up here as a bench regression before it ships.
...
That step is still `continue-on-error: true` in `.github/workflows/ci.yml` (the same "surfaced, non-blocking" status every bun:sqlite suite has, and the workflow's own comments name it the next promotion candidate), so a contract break shows up today as a red, visible, non-blocking result rather than a failed workflow.
Promoting `test:bun` to a hard gate is therefore the single change that turns this contract from documented into enforced, and any cutover using the gate below should treat a red result here as blocking whether or not the workflow agrees yet.
