...
STATUS: SHIPPED in PR #534 (two adversarial rounds; the panel waterfall gained the gen guard the newly-concurrent-path class demands - its third appearance in this arc).
Two discoveries recorded: the luna-config DECIDE above was rewritten at the observe gate (the event is the installer seeding bridge; fresh installs hold the token only in ~/.luna/.env, so consumers cannot re-read what no store holds - Rust now seeds the store directly and the token never crosses the webview boundary), and executing the fan-out path exposed that wiring.ts's hub-event reconnect had NEVER worked in production (bare identifier out of scope since S20c, swallowed by its own catch; root cause @ts-nocheck, evidence attached to #525).
