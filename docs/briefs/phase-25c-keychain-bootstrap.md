...
- macOS keychain entries already exist for `luna.op.antmachine`
  (account `antmachine`). Sterling will add `luna.op.mrbot` and
  `luna.op.flow` manually via `security add-generic-password -U` —
  **do not try to add them yourself**.
- Verified: `bun` reading `luna.op.antmachine` via
...
- Verified: `OP_SERVICE_ACCOUNT_TOKEN=<keychain-token> op vault list`
  returns `Mr Bot` — the antmachine token is real and works.
...
- `/Users/USER/Projects/luna/packages/core/src/secret-provider/index.ts`
...
- Signature:
  ...
  export interface KeychainQuery {
    readonly service: string  // e.g. "luna.op.antmachine"
    readonly account: string  // e.g. "antmachine"
  }
...
- Define a hardcoded list of op service accounts to try, IN
  ...
  ```ts
  const OP_ACCOUNTS = [
    { keychainService: "luna.op.antmachine", keychainAccount: "antmachine" },
    { keychainService: "luna.op.mrbot",      keychainAccount: "mrbot" },
...
- Boot log: `[op] N providers active: <env|antmachine|mrbot|flow>×N`.
...
- Any new `secret_ref` grammar (no `op+mrbot://...`, no
  `op://mrbot@...` — refs stay `op://VAULT/ITEM/FIELD)
...
- A `describe.skipIf(process.platform !== "darwin")` block that
  actually shells out to `security` and reads `luna.op.antmachine`.
  Sterling has this entry pre-populated. Asserts the token starts
...
- With current keychain (only `luna.op.antmachine` populated):
  boot log should read `[op] 1 providers active: antmachine` (env
  not set, mrbot & flow absent).
- With Sol-agent's `OP_SERVICE_ACCOUNT_TOKEN` env set: log should
  read `[op] 2 providers active: env, antmachine` (env first).
- After Sterling adds `luna.op.mrbot` + `luna.op.flow`: log should
  read `[op] 4 providers active: env, antmachine, mrbot, flow` (or
  the subset present).
...
- **Commits along the way** — Sterling explicitly wants
...
- `bun` triggers a GUI prompt when reading the keychain entry —
  ...
  added). Sterling tested this in interactive mode and saw no
