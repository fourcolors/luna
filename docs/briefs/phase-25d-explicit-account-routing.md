...
- Construction-time invariants (bad regex, reserved label, duplicate
  label).
- Single-account chain: `op://v/i/f` resolves; `luna-op://X/v/i/f`
  resolves when X matches; ConfigError when X doesn't.
- Multi-account chain: `op://v/i/f` ConfigErrors with the guidance
  message and lists registered labels; `luna-op://X/...` routes
  only to X.
