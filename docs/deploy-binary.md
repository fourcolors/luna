...
### The extraction rider: a shared update-server test fixture (S22b)

`test/update-server.test.ts` and `apps/deploy-cli/test/update/bash-fixtures.ts` used to carry two independent copies of the same git-checkout-plus-stub-bin fixture, because `makeDeployRepo`/`makeStubBin` were module-private to the first file and the second could not import them.
`test/helpers/update-server-fixtures.ts` is a pure move of `git`/`makeDeployRepo`/`makeStubBin` out of `test/update-server.test.ts` into a shared module, so both suites now build their hermetic fixtures from the same audited pieces.
`test/update-server.test.ts`'s own call sites changed only their import line - this repo's 273-test hostenv suite (`test/deploy-scripts.test.ts` 129 + `test/guardian.test.ts` 90 + `test/update-server.test.ts` 54) is the proof this stayed outcome-identical.
`bash-fixtures.ts`'s own trimmed duplicate of `makeDeployRepo`/`makeStubBin` (about 150 lines, missing several of the option axes the full versions support) is deleted; its `Fixture`/`makeFixture` additionally now expose `serviceName`/`bin`/`systemctlLog` and thread through `isActive`, so the restart-primitive and session-guard parity harness below can reuse the exact same fixture shape instead of growing its own.

## The restart primitive and the session guard (S22b)

This section covers the second of the three PRs S22 splits into: the stop -> settle -> start restart primitive and the fail-closed session guard.
S22c covers readiness/rollback and the releases layout, the remaining piece of the state machine these two pieces sit inside.

`apps/deploy-cli/src/update/restart.ts` and `apps/deploy-cli/src/update/session-guard.ts` port two pieces of `scripts/luna-update-server` that can take a live server down if gotten wrong: the restart primitive whose settle delay exists specifically to avoid `SQLITE_CANTOPEN` from unflushed WAL handles, and the session guard whose fail-closed invariant protects a live operator conversation from an unattended restart.
Both are systemd-supervisor-only ports: the `--user` systemd flag and the launchd supervisor's own stop/start mechanics are out of scope, and `restart.ts`'s own `reload`/`stop`/`start` stay bare-host (no `incus exec` routing) - `session-guard.ts`'s ws-count probe is the one piece that is NOT bare-host-only: it faithfully ports both of `luna_active_ws_count`'s arms (`scripts/lib/luna-deploy.sh:243-289`), host and incus-container, so an incus-targeted guard gets a real answer instead of a manufactured refusal.
Neither module is wired to anything - `scripts/` is untouched, there is no `LUNA_DEPLOY_ENGINE` flag, and `packages/server-registry/src/driver/contract.ts`'s `ServerUpdateDriver` contract diff is empty, matching the frozen-contract decision above.
`restartServiceSync` derives the guard's `dryRun`/`serviceName`/`supervisor` from its own fields rather than accepting them as a second, independently-settable object, so a caller cannot make the guard evaluate a different dry-run/unit/supervisor than the one the primitive actually executes against.
It extends that same non-decoupling to the systemd transport itself: `SessionGuardOptions.readUnitState` (the guard's "count unknown" fallback read) is not accepted from the caller either, and is instead filled by `restartServiceSync` from its OWN `runSystemctl`, so the guard's fallback always interrogates the exact systemd instance `reload`/`stop`/`start` act on.

### `session-guard.ts`: `restartSessionGuardSync`

A port of `restart_session_guard`'s decision procedure (`scripts/luna-update-server:1461-1500`).
The invariant it protects (`scripts/luna-update-server:1440-1442`): the operator is never dropped mid-conversation by an unattended restart.
`GuardVerdict` is a discriminated union, not a flat `permitted: boolean` plus a flat reason union: pairing a deferring reason with `permitted: true` is exactly the bug class this module exists to prevent, so the type makes that pairing unrepresentable.
Four unconditional bash-faithful passthroughs run before the count decision even starts - dry-run, guard-disabled, non-systemd-supervisor, and an explicit operator override - each a verbatim port of one of bash's own unconditional early returns, so they permit on purpose, not as a gap in the invariant.
There is no fifth incus passthrough: an earlier revision of this port deferred unconditionally on any `SessionGuardOptions.incusContainer` (reason `incus-routing-unsupported`) rather than querying it; that refusal was a hedge over an unported half of bash, not a real scope boundary, and is gone.
`queryActiveWsCountSync` now ports BOTH of `luna_active_ws_count`'s arms (`scripts/lib/luna-deploy.sh:243-289`): the bare-host ss(8) probe, and the incus-container arm that execs `sh -c 'command -v ss ...; ss -tnH ...'` inside the named instance byte-for-byte (dev terminates ws connections inside the container, so a host-side check would always read 0 and defeat the guard).
It throws when the count cannot be established on either arm, mirroring the bash function's non-zero return; `restartSessionGuardSync` additionally validates whatever `queryActiveWsCount` returns at the call site (a non-negative integer, or fail closed) so an injected stand-in returning `NaN`, a negative number, or bash's own `"unknown"` sentinel string is treated identically to a thrown query, never as an accepted zero.
The is-active FALLBACK read (`queryUnitStateSync` / a caller's `readUnitState`) stays host-scoped even for an incus target - bash's own fallback routes through `run_target_capture` (`scripts/luna-update-server:365-371`), which execs inside the container, but this port's fallback does not yet; this is a documented gap for a guard used standalone with a container target and an unknown count, left for a future slice alongside actually wiring `restart.ts`'s own `reload`/`stop`/`start` through incus.
Both `queryActiveWsCountSync` and `queryUnitStateSync` are the production defaults for two dependency-injection points, `SessionGuardOptions.queryActiveWsCount` / `.readUnitState`; this module reads no `process.env` at all, so a stray `LUNA_TEST_*` variable inherited by a real process can never spoof the fail-closed decision - the parity harness's TS side injects stub functions instead, while the bash side it compares against keeps using `LUNA_TEST_WS_COUNT`, which is bash's own documented test seam.
`restartSessionGuardSync` catches the count query's throw (or its own invalid-value check tripping) - and, defensively, any throw from the systemd fallback read too - and routes it through the same fail-closed decision bash's own "count unknown" branch makes: permitted only on the one proof that no supervised process can exist (systemd answering `inactive`/`failed`), deferred on every other outcome.
No code path past the passthroughs above, including a thrown or invalid count query, can reach `permitted: true` without one of the two proven-safe outcomes (zero sessions counted, or systemd proving dead-server).

The operator-override log line is a byte-exact observable contract (`scripts/luna-update-server:1467-1469`, the journalctl-of-the-invoking-unit audit trail): `operatorOverrideLogLine` reproduces it byte-for-byte, the operator-override `GuardVerdict` carries it as `auditLine` so a caller cannot grant the bypass without the line in hand to log, and `restart-guard-parity.test.ts` proves the byte match against the real bash script's own captured stderr, not a hand-typed duplicate of the string.
The other bash warn lines inside `restart_session_guard` are informational only and are not documented as a byte-exact contract by this port's own approach; the TS port surfaces them instead as a typed reason (`live-sessions`, `dead-server-exception`, `transport-unreachable`, `unit-state-uncertain`, and so on) that callers can format however they choose.

### `restart.ts`: `settleAfterStopSync` and `restartServiceSync`

`settleAfterStopSync` is a behavioral port of `settle_after_stop` (`scripts/luna-update-server:1256-1286`): same branch structure and the verbatim regex/timing, but the bash `luna_info`/`luna_warn` lines are not reproduced, since this module returns typed outcomes instead - `operatorOverrideLogLine` above is the one genuinely byte-exact artifact this slice ships.
`RESTART_SETTLE_SECS_DEFAULT` carries the verbatim bash default of 6 seconds (`scripts/luna-update-server:99`), with the `SQLITE_CANTOPEN` rationale preserved as a comment: `systemctl stop` returns once the unit's cgroup is torn down, but the outgoing process's DuckDB/SQLite WAL/SHM handles can lag process exit, and a fast restart can open the databases from the new process while those handles are still held.
The settle's raw value stays a string (not a parsed number), so an invalid value like `"not-a-number"` is representable and validated with the same regex `settle_after_stop` uses, degrading to a loud no-op rather than a silent skip - exactly like bash.
The sleep itself is injectable (`sleepSync`) so tests can prove the knob is wired without paying a real 6 seconds of wall-clock time per test; production callers omit it and get the real `sleep` binary at the verbatim default.

`restartServiceSync` is a behavioral port of `restart_service`'s core sequence (`scripts/luna-update-server:1509` the session guard, `1526-1528` `sup_reload`/`sup_stop`/`settle_after_stop`, `1549` `sup_start`): the session guard first, then `daemon-reload` -> `stop` -> settle -> `start`, with the same exit-code contract (`3` on a guard defer, `1` on a supervisor step failing, `0` on success).
Under `dryRun`, `runSystemctl` is never invoked at all for the reload/stop/start steps - they are treated as having succeeded, mirroring bash's `luna_run` (`scripts/lib/luna-deploy.sh:8-18`), which prints the would-be command and returns success without executing it.
The start-limit retry path (`is-failed` -> `reset-failed` -> retry `start`) runs through the same dry-run-gated `runStep` helper as every other call, rather than the raw `runSystemctl`; it is unreachable under `dryRun` regardless (the first `start` never fails in that mode), so the two are behaviorally identical and there is no separate path to explain.
The `start` step includes `sup_start`'s own start-limit-latched recovery (`scripts/luna-update-server:1371-1381`): a unit refused by systemd's `StartLimitIntervalSec`/`StartLimitBurst` latch (`scripts/luna-server-install:280-281`) gets exactly one `is-failed` -> `reset-failed` -> retry-`start` cycle, never touching `reset-failed` ahead of a start that would have succeeded on its own.
Deliberately excluded from this port, and left for S22c: the MainPID pre/post postcondition (`scripts/luna-update-server:1510-1568`'s `pre_pid`/`post_pid` block, entangled with the readiness gate it feeds) and `RESTART_PRESTART_HOOK`, the releases-layout current-symlink flip armed only by deploy/rollback in `--layout releases` mode.
Both are layered on top of the same `reload`/`stop`/`settle`/`start` core this module owns; porting them here would pull readiness-gate and releases-layout logic into a slice scoped to the restart primitive and the session guard alone.
Like `settleAfterStopSync`, the systemctl calls are injected (`runSystemctl`) rather than hardcoded, so this module stays a pure orchestration of an effect a future slice wires for real.

### The parity harness (S22b)

`apps/deploy-cli/test/update/restart-guard-parity.test.ts` reuses the S22a harness patterns - real bash on hermetic fixtures, byte-diff where a byte contract exists, marker comparison elsewhere - against `bash-fixtures.ts`'s `makeFixture`/`runUpdate`, widened to expose `serviceName`/`readinessPort`/`bin`/`systemctlLog` and to thread `isActive` through for the session guard's decision matrix, plus a `makeLightFixture` that skips `makeDeployRepo`'s git init/clone/commit entirely for the scenarios that never call `runUpdate` and only ever consume the stub bin.
`readinessPort` rides on `Fixture` the same way `serviceName` does, so a test building a `SessionGuardOptions` from a fixture can never drift from the `--readiness-port` value pinned into that same fixture's `args`.
For the restart primitive's call order and settle timing, it drives the real bash script and the TS port against the SAME stub `systemctl` binary PLUS an ordered "sleep" stub that appends a `settle` marker into that same log before delegating to the real `sleep` binary, so the diffed sequence (`daemon-reload`/`stop`/`settle`/`start`) pins settle's POSITION, not merely that it happened - verified directly by temporarily moving `restartServiceSync`'s `settleAfterStopSync` call to after its `start` step, confirming the sequence assertion failed (received `start` before `settle`), then reverting.
The TS side additionally asserts its own wall-clock elapsed time against a short real settle - the bash side does not: a bash-side `>=900ms` check was removed because the base run's own process/git/subprocess overhead already clears that bar with settle at `0`, so it could never actually discriminate a missing settle from ordinary overhead; settle's position and presence on the bash side stay pinned by the ordered-log sequence assertion above (and by the settle_after_stop-removed mutant that assertion is verified to kill), not by timing. `RESTART_SETTLE_SECS_DEFAULT` is additionally checked against a regex extracted from `scripts/luna-update-server`'s own `LUNA_RESTART_SETTLE_SECS:-N` default literal, not just a hand-copied `6`.
A separate pair of cases drives `restartServiceSync` with a stubbed ws-count query that always throws, asserting the guard's is-active fallback is observed going through that SAME call's `runSystemctl`, proving the transport non-decoupling described above rather than merely typing it away; two more cases fail `daemon-reload` and `stop` respectively, asserting the primitive stops immediately and - critically for the stop failure - never issues a `start`.
For the session guard, every reachable branch (live sessions, zero sessions, unknown-count with the unit `active`/`failed`/`activating`/empty, a thrown/garbage/non-integer count with no systemd transport reachable at all, a whitespace-only ss(8) line, three CR/space-polluted is-active answers that a bare `.trim()` would wrongly launder into a clean match, the incus-container arm against a fake `incus exec` double, and the operator override) is driven against the real bash script first, and the TS port's verdict for the same stub fixture is asserted to agree with the real exit code bash produced; the four warn-line assertion helpers derive their expected text from `scripts/luna-update-server`'s own source at test runtime rather than a hand-typed copy of it.
The incus double is not merely shaped like `incus exec` - its stubbed `ss` refuses to answer unless it was actually reached through the double's own `exec "$@"`, the only place that sets the witness environment variable the stub checks first, so a mutant that deletes the incus arm entirely (routing straight to the bare-host `ss` call instead) fails the two count-asserting tests instead of passing by coincidence.

## Current state (S21-S22b)
...
- Every subcommand exits `CRITICAL` (`2`) with `deploy-cli <name>: not implemented` on stderr - no deploy logic has been WIRED yet; `apps/deploy-cli/src/update/` ports the transaction-journal and guardian status/health state-file IO (S22a), the stop/settle/start restart primitive, and the fail-closed session guard (S22b) those subcommands will eventually call, but nothing in `main.ts` invokes any of it yet.
