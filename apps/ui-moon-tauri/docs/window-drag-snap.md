...
There used to be a redock override on release: a thread-drawer floater carried an optional `redock` config through `LunaDock.wire`, and the release handler probed a Rust `redock_thread` command before it would snap.
That whole pop-out/redock system was removed along with the drawer drag-out gesture - the chat window's thread sidebar is now a single-window switcher (a row click opens the thread in place), so every release just snaps.
