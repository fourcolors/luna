...
It computes the floater's center in global physical coordinates and runs a scale-independent `center_in_rect` test against the owner's LEFT drawer strip (a fixed ~320 logical px, independent of the now-resizable sidebar's current width, clamped to the window width) rather than the whole owner window, so an accidental drop anywhere over the owner does not trigger it.
