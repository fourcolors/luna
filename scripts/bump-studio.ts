#!/usr/bin/env bun
/**
 * bump-studio.ts — move the Luna Studio version across all four files in lockstep and (optionally)
 * cut the `studio-v*` tag that triggers the macOS CI build+sign+publish pipeline
 * (`.github/workflows/release-studio.yml`).
 *
 * WHY THIS EXISTS
 * The Tauri app's version lives in FOUR files that must always agree:
 *   - apps/ui-studio-tauri/package.json
 *   - apps/ui-studio-tauri/src-tauri/Cargo.toml
 *   - apps/ui-studio-tauri/src-tauri/tauri.conf.json
 *   - apps/ui-studio-tauri/src-tauri/Cargo.lock   (the luna-studio-ui [[package]] entry)
 ...
 *
 * Cargo.lock is a first-class lockstep member from day one, bumped via a
 * package-anchored replace (so only luna-studio-ui's own version moves, never a
 * dependency's).
 *
 * USAGE
 *   bun run scripts/bump-studio.ts --check                 # assert the 4 versions match (CI gate; exit 1 on drift)
 *   bun run scripts/bump-studio.ts <x.y.z>                 # rewrite all 4 to <x.y.z>
 *   bun run scripts/bump-studio.ts <x.y.z> --tag          # also: git commit the 4 files + annotated tag studio-v<x.y.z>
 *   bun run scripts/bump-studio.ts <x.y.z> --tag --push   # also: push the tag -> fires the macOS release CI
 *
 * --push is a "release that publishes to users" — keep it an operator-gated action.
 * The CI button (release-studio.yml workflow_dispatch) drives the bare `<x.y.z>`
...
/** The four files that carry the Studio version and MUST stay in lockstep. */
export const VERSION_FILES: readonly VersionFile[] = [
  { path: "apps/ui-studio-tauri/package.json", kind: "json" },
  { path: "apps/ui-studio-tauri/src-tauri/Cargo.toml", kind: "toml" },
  { path: "apps/ui-studio-tauri/src-tauri/tauri.conf.json", kind: "json" },
  { path: "apps/ui-studio-tauri/src-tauri/Cargo.lock", kind: "lock", pkg: "luna-studio-ui" },
]

export const SEMVER = /^\d+\.\d+\.\d+$/

/* -------------------------------------------------------------------------- */
/* Pure helpers (unit-tested in test/bump-studio.test.ts)                      */
/* -------------------------------------------------------------------------- */
 ...
 * given package's `name = "<pkg>"` line so we move only luna-studio-ui's version,
...
const die = (msg: string): never => {
  process.stderr.write(`bump-studio: ${msg}\n`)
  process.exit(1)
}
...
const main = (): void => {
  ...
  if (flags.has("--check")) {
    ...
    process.stdout.write(`✓ studio version in sync: ${res.distinct[0]}\n`)
    return
  }

  const next = positional[0]
  if (!next) die("usage: bump-studio.ts <x.y.z> [--tag [--push]]  |  bump-studio.ts --check")
  ...
  const commit = git(root, "commit", "-m", `chore(ui-studio-tauri): bump to ${next}`)
  if (!commit.ok) die(`git commit failed: ${commit.out}`)
  const tagged = git(root, "tag", "-a", tag, "-m", `Luna Studio ${next}`)
  ...
  process.stdout.write(`✓ pushed ${tag} — macOS release CI (release-studio.yml) will build + sign + publish.\n`)
}
