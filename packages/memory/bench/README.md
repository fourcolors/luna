...
- `laya`: the local Jev-style classifier (`packages/adapter-sdk/src/laya-reranker.ts`) for operators who want Jev's judging shape without sending memory text to TypeSafe. The reranker replays the same request to a local `laya` sidecar — `pip install laya`, then run `scripts/laya-rerank-server` (127.0.0.1:8182; `LAYA_DEVICE=cuda|mps|cpu`, `LAYA_PRELOAD=1` to keep every checkpoint hot). `LUNA_LAYA_URL` overrides the endpoint and `LUNA_LAYA_MODEL` pins a checkpoint (default `auto`: the sidecar's Router picks per request). Same opt-in semantics as `jev`: both lanes rerank unless their flag is `0`, at depth 40 with no threshold (`LUNA_RERANK_LAYA_MAX_CANDIDATES` and `LUNA_RERANK_LAYA_THRESHOLD` override).
