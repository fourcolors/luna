...
- `laya`: the local Jev-style classifier (`packages/adapter-sdk/src/laya-reranker.ts`) for operators who want Jev's judging shape without sending memory text to TypeSafe. The reranker replays the same request to a local `laya` sidecar — `scripts/laya-env install` then `scripts/laya-env start` sets it all up (python≥3.10 venv at `~/.luna/laya-env`, `laya` package, detached `scripts/laya-rerank-server` on 127.0.0.1:8182; `LAYA_DEVICE=cuda|mps|cpu`, `LAYA_PRELOAD=1` to keep every checkpoint hot). Moon's Models panel shows a live up/down indicator for it — the server polls `<LUNA_LAYA_URL>/health` and broadcasts the verdict on `model-routing-list` frames. `LUNA_LAYA_URL` overrides the endpoint and `LUNA_LAYA_MODEL` pins a checkpoint (default `auto`: the sidecar's Router picks per request). Same opt-in semantics as `jev`: both lanes rerank unless their flag is `0`, at depth 40 with no threshold (`LUNA_RERANK_LAYA_MAX_CANDIDATES` and `LUNA_RERANK_LAYA_THRESHOLD` override).

Each engine's depth and threshold are tuned to that engine, so the overrides are per engine: `LUNA_RERANK_<ENGINE>_THRESHOLD` (0-100, the injection threshold for both lanes) and `LUNA_RERANK_<ENGINE>_MAX_CANDIDATES`, where `<ENGINE>` is `CE`, `JEV`, or `LAYA`. The older unscoped `LUNA_RERANK_THRESHOLD` and `LUNA_RERANK_MAX_CANDIDATES` still work, but only for the cross-encoder. With any other engine they are ignored and the server logs a warning once, so cross-encoder tuning left in `.env` can't silently cut Jev to 8 candidates with a threshold of 40.
