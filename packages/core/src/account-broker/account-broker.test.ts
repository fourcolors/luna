...
describe("AccountBroker.add / remove (hot pool mutate)", () => {
  it("add then list then remove updates the live pool", async () => {
    const out = await Effect.runPromise(
      Effect.gen(function* () {
        const broker = yield* AccountBroker
        ...
        const mid = yield* broker.list("anthropic")
        yield* broker.remove("account-secondary-1")
        const after = yield* broker.list("anthropic")
        return { before, mid, after }
      }).pipe(Effect.provide(buildLayer(1000))),
    )
    expect(out.before).toHaveLength(3)
    expect(out.mid.map((a) => a.id)).toContain("account-secondary-1")
    expect(out.mid.find((a) => a.id === "account-secondary-1")).toMatchObject({
      label: "secondary",
      kind: "anthropic",
      health: "healthy",
    })
    expect(out.after.map((a) => a.id)).not.toContain("account-secondary-1")
    expect(out.after).toHaveLength(3)
  })

  it("rejects duplicate id and invalid secret-ref", async () => {
    const out = await Effect.runPromise(
      Effect.gen(function* () {
        const broker = yield* AccountBroker
        ...
        const badRef = yield* Effect.exit(
          broker.add({
            id: "new-one",
            label: "x",
            kind: "anthropic",
    ...
    expect(Exit.isFailure(out.dup)).toBe(true)
    expect(Exit.isFailure(out.badRef)).toBe(true)
  })
})

describe("AccountBroker.list health spent vs rate_limited", () => {
  it("budget exhaustion surfaces health='spent'", async () => {
    const budgetSeeds: ReadonlyArray<AccountSeed> = [
      { id: "a1", kind: "anthropic", secretRef: "anth:a1", budgetUsd: 0.000001 },
    ]
    const out = await Effect.runPromise(
      Effect.gen(function* () {
        const broker = yield* AccountBroker
        yield* Effect.scoped(broker.acquireSession({ model: "claude-sonnet-4-5" }))
        yield* broker.report({
          accountId: "a1",
          kind: "usage",
          model: "claude-sonnet-4-5",
      ...
      }).pipe(Effect.provide(buildLayer(1000, budgetSeeds))),
    )
    expect(out.find((a) => a.id === "a1")?.health).toBe("spent")
  })
})
