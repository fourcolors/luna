 ...
 * @anthropic-ai/claude-agent-sdk@0.3.202 sdk.mjs model catalog):
 *   - Haiku / Mythos / Opus 4.0–4.5 → no effort (no effort capability in SDK).
 *   - Fable / Opus 4.7 / Opus 4.8 / Sonnet 5 → all five levels including
 *     xhigh (SDK: xhigh_effort capability present for these models).
 ...
 * first-party only (no bedrock/vertex), so no effort param.
 */
export const effortsForModel = (id: string): ReadonlyArray<EffortLevel> => {
  const m = id.toLowerCase()
  if (/haiku|mythos/.test(m)) return []
  // xhigh-capable: fable, opus-4-7/4-8, sonnet-5 (all have xhigh_effort in SDK)
  if (/fable|opus-4-(7|8)|opus-4\.?[78]|sonnet-5/.test(m)) return EFFORT_LEVELS
...
export const defaultEffortForModel = (id: string): EffortLevel | undefined => {
  const m = id.toLowerCase()
  // SDK: claude-fable-5 and claude-sonnet-5 both have default_effort: "high"
  if (/fable|sonnet-5/.test(m)) return "high"
  return undefined
}
