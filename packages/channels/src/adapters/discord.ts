    ...
    onMessage: (cb) => {
      client.on(Events.MessageCreate, (m) => {
        try {
          ...
          console.warn(
            "[luna/channels] discord: inbound normalization failed:",
...
const ingestDiscordAttachments = async (
  ...
  for (const a of attachments) {
    ...
    try {
      ...
      // 3a A5 discipline).
      console.warn(
        "[luna/channels] discord: attachment download failed (turn continues):",
        err instanceof Error ? err.message : String(err),
      )
...
export const makeDiscordAdapter = (config: DiscordAdapterConfig): ChannelAdapter => {
   ...
   */
  const typingFibers = new Map<string, Fiber.Fiber<void, never>>()
  ...
  const noteDrop = (m: InboundDiscordMessage, kind: "message" | "interaction"): void => {
    ...
    loggedDrops.add(key)
    console.warn(
      `[luna/channels] discord: dropped inbound ${kind} from user=${m.authorId} ` +
  ...
  const attemptTyping = (t: DiscordTransport, channelId: string): Effect.Effect<void> =>
    Effect.gen(function* () {
      ...
      console.warn(
        `[luna/channels] discord: typing indicator disabled for channel=${channelId} — ` +
    ...
    try {
      ...
      if (ref !== undefined && !isCommandText) {
        if (ref.channelId !== m.channelId) {
          ...
          console.warn(
            "[luna/channels] discord: reply quote skipped: cross-channel reference " +
          ...
          if (rt !== null) {
            try {
              ...
              console.warn(
                "[luna/channels] discord: reply reference fetch failed (quote skipped, turn continues):",
                err instanceof Error ? err.message : String(err),
              )
            }
          }
      ...
      if (refusals.length > 0) {
        ...
        await t.send(m.channelId, attachmentRefusalNote(refusals)).catch((err) => {
          console.warn(
            "[luna/channels] discord: attachment refusal note failed:",
            err instanceof Error ? err.message : String(err),
          )
        })
      }
      ...
      console.warn(
        "[luna/channels] discord: inbound handler error:",
        err instanceof Error ? err.message : String(err),
      )
    }
  }
  ...
  const handleInteraction = async (i: InboundDiscordInteraction): Promise<void> => {
    try {
      ...
      await t.ackInteractionEphemeral(i.id, i.token, "On it.").catch((err) => {
        console.warn(
          "[luna/channels] discord: interaction ack failed (turn continues):",
      ...
      console.warn(
        "[luna/channels] discord: inbound interaction handler error:",
        err instanceof Error ? err.message : String(err),
      )
    }
  }
  ...
  const registerCommandsOnReady = async (t: DiscordTransport): Promise<void> => {
    ...
    if (guildId === undefined || guildId === "") {
      ...
      console.log(
        "[luna/channels] discord: slash commands: not registered (no guildId configured; " +
    ...
    try {
      ...
      // silently rot it.
      console.log(
        `[luna/channels] discord: discord slash commands: registered guild=${guildId} count=${discordCommandManifest.length}`,
      )
    } catch (err) {
      console.warn(
        `[luna/channels] discord: discord slash commands registration failed guild=${guildId} ` +
  ...
  const withRateLimitRetry = <A>(
    ...
    Effect.gen(function* () {
      for (let attempt = 1; attempt <= MAX_RATE_LIMIT_ATTEMPTS; attempt++) {
        ...
        if (retryMs === null) {
          console.warn(`[luna/channels] discord: ${label} failed:`, err instanceof Error ? err.message : String(err))
          return null
        }
        if (attempt === MAX_RATE_LIMIT_ATTEMPTS) {
          console.warn(`[luna/channels] discord: ${label} rate-limited, attempts exhausted`)
          return null
        }
    ...
    start() {
      return Effect.gen(function* () {
        ...
        if (!started) {
          ...
          t.onError((e) => {
            console.warn("[luna/channels] discord: client error:", e.message)
          })
          t.onReady((tag) => {
            if (logLogin) {
              console.log(`[luna/channels] discord: logged in as ${tag}`)
              console.log(
                `[luna/channels] discord: allowlist: ${allowedUsers.size} user(s), ` +
        ...
        yield* Effect.tryPromise({ try: () => t.login(), catch: (e) => e as unknown }).pipe(
          Effect.catch((e) => {
            console.warn(
              "[luna/channels] discord: login failed:",
