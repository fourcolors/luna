...
describe("end-to-end: TelegramAdapter + ChannelService", () => {
  ...
  it("restart: after stopAdapters() → startAdapters(), an allowed inbound still gets a typing action", async () => {
    ...
    //
    // The assertions are ORDERED on purpose (task #8): the service-layer
    // landmine (stopAdapters never clearing startedAdapterIds → restart is a
    // silent no-op) fires FIRST; only once the restart actually happens can
    // the adapter-level typingSwept latch (set in stop(), never reset) be
    // observed. A failure on `starts` is the service bug; a failure on the
    // typing count is the adapter bug.
