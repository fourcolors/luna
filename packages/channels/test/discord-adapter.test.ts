...
describe("Slice 1 — typing indicator: refresh, cap and stop", () => {
  ...
  it("stop() sweeps a live typing fiber", async () => {
    ...
    try {
      await Effect.runPromise(
        Effect.gen(function* () {
          const fiber = yield* Effect.forkChild(Effect.scoped(s.adapter.start()))
          yield* Effect.sleep(Duration.millis(20))
          yield* Effect.promise(async () => {
...
describe("Slice 1 — sendTyping failure breaker (per channel)", () => {
  ...
  it("logs exactly ONE warning when a channel's breaker opens", async () => {
    ...
    const hits = warnings.filter(
      (w) => w.includes("[luna/channels] discord:") && /typing/i.test(w) && w.includes("chan-a"),
    )
  ...
  it("WHEN registration fails THEN the failure is LOUD, primitive-fields-only (A5: no err.name, no raw error object), and the adapter stays up", async () => {
    ...
    try {
      ...
      await withStarted(s.adapter, async () => {
        ...
        // LOUD: exactly one warning, naming the feature and the guild. NOT the
        // donor's swallow (Sol logs and moves on with no guild id and at
        // .message granularity only when it remembers) and NOT a crash.
        // console.WARN, not error: packages/channels logs at warn level only
        // (telegram.ts has 6 console.warn and 0 console.error).
        const regCallsLogged = warnSpy.mock.calls.filter((c) =>
          /slash commands/.test(c.map((a) => String(a)).join(" ")),
        )
        expect(regCallsLogged, "exactly one failure line").toHaveLength(1)
        const line = regCallsLogged[0]?.map((a) => String(a)).join(" ") ?? ""
        ...
        for (const arg of regCallsLogged[0] ?? []) {
          expect(
            ["string", "number", "boolean"].includes(typeof arg),
            `console.warn arg is a primitive, got ${typeof arg}`,
          ).toBe(true)
        }
