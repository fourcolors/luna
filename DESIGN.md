...
The compile-time `@ts-expect-error` proof in `packages/memory-tools/test/layer.test.ts` (that a search-less backend can no longer satisfy `makeMemoryRouterLayer`) is not mechanically gated today: `packages/memory-tools` has no `tsconfig.json`, the root `tsconfig.json` excludes `**/*.test.ts`, and CI runs only the root `bun run typecheck` step - so the constraint is real and currently green, but nothing in this repo's gates would go red if it regressed.
Closing that gap needs a `packages/memory-tools/tsconfig.json` plus a CI step that runs it, both outside S04's file scope; until then, treat the `@ts-expect-error` line as a documented, manually-verified invariant rather than a CI-enforced one.
