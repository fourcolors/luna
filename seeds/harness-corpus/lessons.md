...
incus-fence-acl-gateway  | covered | probes/060-incus-fence-acl-gateway-guard.sh  | incus emits broad reject rule before narrow allow-gateway rule; if gateway falls inside reject range all container-to-gateway traffic is dropped (EmbedderError crash-loop); fix: carve gateway out of reject destination
```
